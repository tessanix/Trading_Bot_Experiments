...
def minMaxNorm(_min, _max, _data):
    return (_data-_min)/(_max-_min)  

class Agent:
    def __init__(self, base_dir, chkpt_dir, transParams,
                 alpha=0.0001, gamma=0.8, gae_lambda=0.95, policy_clip=0.2, batch_size=64, n_epochs=10
                ):
        ...
        self.actor = ActorNetworkTransformerCategorical(base_dir=base_dir, chkpt_dir=chkpt_dir, name='actor_ppo.weights.h5', encoderParams=transParams) 
        self.critic = CriticNetworkTransformerCategorical(base_dir=base_dir, chkpt_dir=chkpt_dir, name='critic_ppo.weights.h5', encoderParams=transParams) 
